    ...
    <div className="flex h-screen w-full bg-[#f6f6f8]">
      ...
      <aside className="w-64 flex-shrink-0 bg-white border-r border-slate-200 flex flex-col">
        ...
        <div className="flex-1 px-4 py-2 space-y-1 overflow-y-auto">
          ...
          <SidebarItem to="/" icon="dashboard" label="Dashboard" active={path === '/'} />
          <SidebarItem to="/atendimentos" icon="groups" label="Atendimentos" active={path === '/atendimentos'} />
          
          <p className="px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 mt-6">Gestão</p>
          <SidebarItem to="/financeiro" icon="payments" label="Financeiro" active={path === '/financeiro'} />
          <SidebarItem to="/documentos" icon="folder_open" label="Documentos" active={path === '/documentos'} />
          <SidebarItem to="/servicos" icon="description" label="Serviços e Atos" active={path === '/servicos'} />
          
          <p className="px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 mt-6">Sistema</p>
          <SidebarItem to="/usuarios" icon="manage_accounts" label="Usuários" active={path === '/usuarios'} />
          <SidebarItem to="/configuracoes" icon="settings" label="Configurações" active={path === '/configuracoes'} />
        </div>

        <div className="p-4 border-t border-slate-200">
          <div className="flex items-center gap-3">
             <div className="size-9 rounded-full bg-gray-200 bg-[url('https://i.pravatar.cc/150?u=a042581f4e29026024d')] bg-cover bg-center"></div>
