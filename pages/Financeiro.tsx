  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Controle Financeiro</h1>
          <p className="text-slate-500">Caixa Diário - Acompanhe as movimentações.</p>
        </div>
        <div className="flex gap-2">
           <button className="flex items-center gap-2 bg-white border border-slate-300 text-slate-700 font-bold text-sm px-4 py-2 rounded-lg hover:bg-slate-50">
             <span className="material-symbols-outlined text-sm">calendar_today</span>
             Hoje: 17 de Julho
           </button>
           <button className="bg-primary text-white font-bold text-sm px-4 py-2 rounded-lg hover:bg-primary/90">
             Fechar Caixa
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl border border-slate-200">
           <p className="text-slate-500 font-medium mb-1">Saldo Inicial</p>
           <p className="text-2xl font-bold text-slate-900">R$ 5.120,00</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-200">
           <p className="text-green-600 font-medium mb-1">Total Entradas</p>
           <p className="text-2xl font-bold text-slate-900">R$ 8.750,50</p>
        </div>
        <div className="bg-white p-6 rounded-xl border border-slate-200">
           <p className="text-red-600 font-medium mb-1">Total Saídas</p>
        ...
        <div className="bg-primary/10 border border-primary/20 p-6 rounded-xl">
           <p className="text-primary font-bold mb-1">Saldo Atual</p>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
           <nav className="flex gap-6 text-sm font-bold">
             <button className="text-primary border-b-2 border-primary pb-4 -mb-4.5">Movimentações do Dia</button>
             <button className="text-slate-500 hover:text-slate-700">Contas a Pagar</button>
             <button className="text-slate-500 hover:text-slate-700">Contas a Receber</button>
             <button className="text-slate-500 hover:text-slate-700">Relatórios</button>
           </nav>
           <div className="flex gap-2">
              <button className="flex items-center gap-1 bg-primary text-white text-xs font-bold px-3 py-2 rounded-lg hover:bg-primary/90">
                <span className="material-symbols-outlined text-sm">add</span> Lançar Receita
              </button>
              <button className="flex items-center gap-1 bg-slate-100 text-slate-700 text-xs font-bold px-3 py-2 rounded-lg hover:bg-slate-200">
